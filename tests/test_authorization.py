...
def test_authorization(server_start, send_message, orm):
    send_message.Register(RegisterRequest(user_name=USER, user_passwd=PASSWD))
    response = send_message.Login(
        LoginRequest(user_name=USER, user_passwd=PASSWD)
    )
...
def test_authorization_error(send_message, orm):
    ...
    send_message.Register(RegisterRequest(user_name=USER, user_passwd=PASSWD))
    response_bad_PASSWD = send_message.Login(
        LoginRequest(user_name=USER, user_passwd="")
    )
